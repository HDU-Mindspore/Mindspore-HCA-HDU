...
def test_sin_std(mode):
    """
    Feature: standard forward, backward features.
    Description: test function sin.
    ...
    if mode == 'pynative':
        ...
        output_grad = sin_backward_func(ms.Tensor(x))
    else:
        output = (jit(sin_forward_func, backend="ms_backend", jit_level="O0"))(ms.Tensor(x))
        output_grad = (jit(sin_backward_func, backend="ms_backend", jit_level="O0"))(ms.Tensor(x))
