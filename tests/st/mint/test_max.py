...
def max_forward_func(x):
    return mint.max(x)


def max_backward_func(x):
    return ops.grad(max_forward_func, (0,))(x)


@arg_mark(plat_marks=['cpu_linux'], level_mark='level2', card_mark='onecard', essential_mark='essential')
@pytest.mark.parametrize('mode', ['pynative'])
def test_max_std(mode):
    """
    Feature: standard forward, backward features.
    Description: test function max.
    ...
    if mode == 'pynative':
        ...
        output = max_forward_func(ms.Tensor(x))
        output_grad = max_backward_func(ms.Tensor(x))
    else:
        output = (jit(max_forward_func, backend="ms_backend", jit_level="O0"))(ms.Tensor(x))
